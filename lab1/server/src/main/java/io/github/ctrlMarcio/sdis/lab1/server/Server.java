...
import io.github.ctrlMarcio.sdis.lab1.server.command.LookupCommand;
import io.github.ctrlMarcio.sdis.lab1.server.command.RegisterCommand;
    ...
    public void run() {
        ...
        while (this.running) {
            try {
                ...
                response.send(socket, packet.getAddress(), packet.getPort());
            } catch (IOException e) {
                e.printStackTrace();
                this.running = false;
            }
